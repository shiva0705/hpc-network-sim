...
void mpi_init( mpi_process * p, tw_lp * lp)
{
    tw_event *e;
    tw_stime ts;
    nodes_message *m;
...
void dimension_order_routing( nodes_state * s, tw_lpid * dst_lp, int * dim, int * dir )
{
  int dim_N[ N_dims ], 
      dest[ N_dims ],
      i;
...
void update_waiting_list( nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp )
{
...
void waiting_packet_free(nodes_state * s, int loc)
{
...
void mpi_event_rc_handler( mpi_process * p, tw_bf * bf, nodes_message * msg,tw_lp * lp)
{
 switch(msg->type)
  {
     case MPI_SEND:
...
int main(int argc, char **argv, char **env)
{
    ...
    {
	    MPI_Reduce( &N_finished_storage[i], &total_finished_storage[i],1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
	    MPI_Reduce( &N_generated_storage[i], &total_generated_storage[i],1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
	    MPI_Reduce( &N_num_hops[i], &total_num_hops[i],1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
	    MPI_Reduce( &N_queue_depth[i], &total_queue_depth[i],1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
    }
